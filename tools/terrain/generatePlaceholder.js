...
// tools/terrain/README.md) to replace these files with the real dataset for a location.
//
// Generates placeholders for every configured location by default (see
// resolveLocationSlugs in config.js) — pass --location=<slug> to generate just one.
...
// Illustrative real-world reference points only (Cut Gate's actual summit/valley
// elevations) — not used for anything but shaping the synthetic profile so it's roughly
// the right scale for a Peak District descent; other locations reuse the same numbers
// until their own real data replaces the placeholder.
...
function placeholderNoticeFor(location) {
  return (
    'PLACEHOLDER DATA — synthetic, not derived from any real survey. Run `npm run ' +
    `terrain:build\` (see tools/terrain/README.md) to generate the real ${location.name} dataset.`
  );
}

function generatePlaceholderFor(location) {
  const origin = localOriginOf(location);
  const paths = outputPathsFor(location.slug);
  const notice = placeholderNoticeFor(location);
  ...
  mkdirSync(dirname(fileURLToPath(paths.terrain)), { recursive: true });
  mkdirSync(dirname(fileURLToPath(paths.route)), { recursive: true });
  mkdirSync(dirname(fileURLToPath(paths.paths)), { recursive: true });
  mkdirSync(dirname(fileURLToPath(paths.trees)), { recursive: true });
  writeFileSync(paths.terrain, JSON.stringify(terrainData));
  writeFileSync(paths.route, JSON.stringify(routeData));
  writeFileSync(paths.landcover, JSON.stringify(landcoverData));
  writeFileSync(paths.paths, JSON.stringify(pathsData));
  writeFileSync(paths.trees, JSON.stringify(treesData));

  console.log(
    `[${location.slug}] Wrote placeholder terrain (${cols}x${rows} @ ${cellSize}m) and route ` +
      `(${routePoints.length} points).`,
  );
  console.log(`[${location.slug}] Landcover class histogram:`, landcoverHistogram);
  console.log(`[${location.slug}] Wrote placeholder paths (${pathsData.paths.length} segments).`);
  console.log(`[${location.slug}] Wrote placeholder trees (${trees.length} trees).`);
}

for (const slug of resolveLocationSlugs()) {
  generatePlaceholderFor(getLocation(slug));
}
console.log('Reminder: this is synthetic placeholder data, not real route topology.');
